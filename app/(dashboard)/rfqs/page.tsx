...
import { listRfqs, listVendorsForRfqForm } from "@/features/rfqs/services/rfq.service";
...
export default async function RfqsPage({ searchParams }: RfqsPageProps) {
  ...
  const [params, canCreate] = await Promise.all([resolveSearchParams(searchParams), hasPermission(session, "rfq.create")]);

  const q = getStringParam(params, "q");
  const statusParam = getStringParam(params, "status");
  const status = Object.values(RfqStatus).find((value) => value === statusParam);
  ...
  const [result, vendors] = await Promise.all([
    listRfqs(session, { q, status, vendorId, page, pageSize }),
  ...
  return (
    <section className="space-y-5">
      <PageHeader
        title="RFQ List"
        description="Manage request-for-quotation lifecycle, items, and vendor invitations."
        action={canCreate ? { label: "New RFQ", href: "/rfqs/new" } : undefined}
      />
      ...
      <form className="grid gap-3 rounded-2xl border border-[var(--border)] bg-white p-3 shadow-sm md:grid-cols-4" action="">
        <input
          ...
          name="q"
          defaultValue={q}
          placeholder="Search by RFQ number, title, vendor"
        ...
        <select name="status" defaultValue={status ?? ""} className="h-9 rounded-md border border-[var(--border)] px-3 text-sm">
          ...
          {Object.values(RfqStatus).map((statusValue) => (
      ...
      {result.rfqs.length === 0 ? (
        <EmptyState title="No RFQs found" description="Try adjusting filters or create a new RFQ." />
      ) : (
        <>
          <RfqsTable rfqs={result.rfqs} />
          <div className="flex flex-wrap items-center justify-between gap-3 text-sm">
            <p className="text-[var(--muted)]">
              Page {result.page} of {result.totalPages} ({result.total} RFQs)
            </p>
