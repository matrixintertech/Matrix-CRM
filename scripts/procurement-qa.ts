...
async function main() {
  ...
  try {
    ...
    const statusUpdatedVendor = await updateVendorStatus(updatedVendor.id, VendorStatus.ACTIVE, true);
    ...
    const vendorList = await listVendors(companyAdminSession as never, { q: "QAV-", page: 1, pageSize: 20 });
    pushResult(results, "vendors.search_vendor", vendorList.vendors.length >= 2);
    ...
    const deletedVendor = await softDeleteVendor(secondVendor.id);
